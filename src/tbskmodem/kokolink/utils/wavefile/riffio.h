...
namespace TBSKmodemCPP
{
    ...
    public:
        ...
        Chunk(const char* name, int size);
    ...
    protected:
        const size_t _data;
    public:
        RawChunk(const char* name, int size, IBinaryReader& fp);
        RawChunk(const char* name, const TBSK_BYTE* data, int data_len);
        virtual ~RawChunk() {}
    public:
    ...
    public:
        ...
        //{
        //}
        DataChunk(int size, IBinaryReader& fp);
        DataChunk(const TBSK_BYTE* data, int data_len);
    ...
    public:
        ...
        ChunkHeader(const char* name, int size, IBinaryReader& fp);
        ChunkHeader(const char* name, int size, const char* form);
        virtual ~ChunkHeader() {};
        const char* GetForm();
    };
    ...
    public:
        ...
        RiffHeader(int size, const char* form);
    };
    // """下位構造をそのまま格納するLIST
    // """
    ...
    public:
        ...
        RawListChunk(const char* form, const char* payload, int payload_len);
...
{
public:
    std::vector<std::shared_ptr<const Chunk>> _chunks;
...
public:
    ...
    WaveFile(int samplerate, int samplewidth, int nchannel, const TBSK_BYTE* frames, int frames_len, std::vector<std::shared_ptr<const Chunk>>& extchunks);
    WaveFile(int samplerate, int samplewidth, int nchannel, std::vector<TBSK_BYTE>& frames, std::vector<std::shared_ptr<const Chunk>>& extchunks) :
        WaveFile(samplerate, samplewidth, nchannel, (const TBSK_BYTE*)frames.data(), (int)frames.size(), extchunks) {}
